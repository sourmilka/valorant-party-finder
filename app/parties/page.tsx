...
import { PartyInvite, FilterOptions } from '@/types';

export default function BrowsePartiesPage() {
  ...
  const regions = ['NA', 'EU', 'AP', 'BR', 'KR', 'LATAM'];
  const ranks = ['Iron', 'Bronze', 'Silver', 'Gold', 'Platinum', 'Diamond', 'Immortal', 'Radiant'];
  const modes = ['Ranked', 'Unrated', 'Spike Rush', 'Deathmatch', 'Escalation', 'Replication'];
  const sizes = ['Solo', 'Duo', 'Trio', 'FourStack'];
  const sortOptions = [
    ...
    try {
      ...
      if (data.success) {
        if (page === 1) {
          ...
          setParties(prev => [...prev, ...data.data.parties]);
        }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-valorant-light/80 text-lg max-w-2xl mx-auto">
            Find the perfect party to join. Filter by rank, region, mode, and more.
          </p>
        </motion.div>
                  ...
                  <div className="space-y-2">
                    {regions.map((region) => (
                      <label key={region} className="flex items-center">
                        ...
                        />
                        <span className="ml-2 text-valorant-light/80">{region}</span>
                      </label>
                    ))}
                  </div>
                </div>
                    ...
                    {modes.map((mode) => (
                      <label key={mode} className="flex items-center">
                        <input
                          ...
                          className="h-4 w-4 text-valorant-red focus:ring-valorant-red border-gray-600 rounded"
                        />
                        <span className="ml-2 text-valorant-light/80">{mode}</span>
                      </label>
                    ))}
                  </div>
                </div>
                    ...
                    {sizes.map((size) => (
                      <label key={size} className="flex items-center">
                        <input
                          ...
                          className="h-4 w-4 text-valorant-red focus:ring-valorant-red border-gray-600 rounded"
                        />
                        <span className="ml-2 text-valorant-light/80">{size}</span>
                      </label>
                    ))}
                  </div>
                </div>
        ...
        >
          <div className="card text-center">
            ...
            <div className="text-valorant-light/60 text-sm">Active Parties</div>
          </div>
          <div className="card text-center">
            <MapPin className="w-6 h-6 text-valorant-red mx-auto mb-2" />
            <div className="text-2xl font-bold text-white">{regions.length}</div>
            <div className="text-valorant-light/60 text-sm">Regions</div>
          </div>
          <div className="card text-center">
